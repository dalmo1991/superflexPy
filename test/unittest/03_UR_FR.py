...
class TestHRU(unittest.TestCase):
    """
    This class tests the functionality of the HRU. The single components have
    been already tested separately in the tests 01 and 02.
    ...
    def _read_inputs(self):
        ...
        data = pd.read_csv('{}/test/reference_results/03_UR_FR/input.dat'.format(package_path), header = 6, sep = '\s+|,\s+|,', engine = 'python')
        self._precipitation = data.iloc[:,6].values
        self._pet = data.iloc[:,7].values

    def _read_outputs(self):
        self._superflex_output = pd.read_csv('{}/test/reference_results/03_UR_FR/Results.csv'.format(package_path))

    def _test_hru_start_stop(self, solver):
    ...
    def test_hru_start_stop_python(self):

        self._test_hru_start_stop(solver = 'python')

    def test_hru_start_stop_numba(self):

        self._test_hru_start_stop(solver = 'numba')
