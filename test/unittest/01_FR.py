...
class TestFR(unittest.TestCase):
    ...
    def _read_inputs(self):
        ...
        data = pd.read_csv('{}/test/reference_results/01_FR/input.dat'.format(package_path), header=6, sep='\s+|,\s+|,', engine='python')
        self._precipitation = data.iloc[:, 6].values
        self._pet = data.iloc[:, 7].values

    def _read_outputs(self):
        self._superflex_output = pd.read_csv('{}/test/reference_results/01_FR/Results.csv'.format(package_path))
