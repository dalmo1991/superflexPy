...
In this page shows how to build a complete semi-distributed conceptual model
with SuperflexPy, including:
...
All the models developed in this section are made available as runnable examples
(see :ref:`examples`).
...
differential equation of the reservoir, and lines 5-7 import the components of
...
the equations used and the import path, is available in page
:ref:`elements_list`. If the desired element is not available, it can be built
following the instructions given in page :ref:`build_element`.
...
implementation (i.e. not Numba) of the implicit Euler (numerical approximator)
and the Pegasus algorithm (root finder). This can be done with the following
code, where the default settings of the solver are used (refer to the solver
docstring).
...
the model) is automatically set equal to the length of the input arrays.
...
The state of the reservoir at all the time steps is saved in the
:code:`state_array` attribute of the element and can be accessed
...
initialization. The states of the model can be reset calling using the method
:code:`reset_states`.
...
After initialization, time step and inputs are defined
...
The unit sets the time step for all its elements and transfers the inputs to the
...
The node is now initialized, combining the two units
...
The same time step will be specified for the elements within the node, and the
inputs will be passed to all the units of the node.
...
to a node the states of the elements belonging to them remain independent while
the parameters stay linked, meaning that the change of a parameter in
:code:`unit-1` in :code:`node-1` is applied also to :code:`unit-1` in
:code:`node-2`. As noted in section :ref:`components_node`, different behavior
can be achieved setting the parameter :code:`shared_parameters` equal to
:code:`False` when initializing the nodes.
